...
function createpost(req, res, next) {
    ...
    MediaObjects_1.Media.create(mediaobj)
        .then(() => Promise.resolve(Posts_1.Posts.create({ id: postid, type: req.body.type, list_of_media: [full_mediaobj] })))
        .then(() => {
...
function update_a_post(req, res, next) {
    ...
    if (req.body.id) {
        Posts_1.Posts.update({ id: req.body.id }, getUpdatepostParams(req.body)).then((data) => {
            let tags = [];
